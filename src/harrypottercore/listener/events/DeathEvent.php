...
class DeathEvent implements Listener{
    ...
    public function playerEffects(Player $player) {
	        if($player instanceof Player) { 
	   	$config = new Config($this->plugin->playerFolder . $player->getLowerCaseName() . ".yml", Config::YAML);
		$config->set("death", $this->plugin->getDeath($player) + 1);
		$config->save();
		$level = (int) $this->plugin->getXpLevel($player);
		$this->roundedlevel = round($level / 10);
		}
    }
	
    public function killerEffects(Player $killer) {
	    if($killer instanceof Player) {
	       $kconfig = new Config($this->plugin->playerFolder . $killer->getLowerCaseName() . ".yml", Config::YAML);
	       $kconfig->set("playerkilled", $this->plugin->getPlayerKilled($killer) + 1);
	       $kconfig->save();
	       $killer->addXpLevel($this->roundedlevel);
               TokenAPI::getInstance()->addToken($killer, 1); 
	       }
    }
		
    public function onDeathByHuman(PlayerDeathEvent $event){
	    $event->setXpDropAmount(0);
	    $event->setKeepInventory(true);
	    $player = $event->getPlayer();
	    $p = $player->getName();
	    $entity = $event->getEntity();
	    if($player instanceof Player and $entity instanceof Human and $entity instanceof Entity){
	       $cause = $entity->getLastDamageCause();
               if ($cause instanceof ProjectileHitEntityEvent) {
		   $killer = $cause->getDamager();
		   if($killer instanceof Human) { 
		      $killername = $killer->getName();
             	      $killer->sendPopup(Translation::getMessage("gainToken", ["player" => $p]));
            	      $event->setDeathMessage(Translation::getMessage("playerDeath", ["player" => $p, "killer" => $killername, "item" => $this->plugin->item])); 
		      $this->killerEffects($killer);
		      $this->playerEffects($player);
		   }
	       }
	       if($cause instanceof EntityDamageByEntityEvent) { 
		  $killer = $cause->getDamager();
		  if($killer instanceof Human) { 
		     $weapon = $killer->getInventory()->getItemInHand()->getCustomName();
		     $killername = $killer->getName();
             	     $killer->sendPopup(Translation::getMessage("gainToken", ["player" => $p]));
            	     $event->setDeathMessage(Translation::getMessage("playerDeath", ["player" => $p, "killer" => $killername, "item" => $weapon])); 
		     $this->killerEffects($killer);
		     $this->playerEffects($player);
		  }
	       }
    ...
    public function onDeathByMob(PlayerDeathEvent $event){
	    $event->setXpDropAmount(0);
	    $event->setKeepInventory(true);
	    $entity = $event->getEntity();
	    $player = $event->getPlayer();
	    $name = $entity->getName();
	    $p = $player->getName();
	    if($entity instanceof Entity and $player instanceof Player){
	       $cause = $entity->getLastDamageCause();
               if ($cause instanceof EntityDamageByEntityEvent) {
	 	   $this->plugin->getServer()->broadcastMessage(Translation::getMessage("broadcastDeath", ["player" => $p, "killer" => $name]));
	       }
	    }
    }
}
